...
class Upload {
    ...
    public function upload($files='') {
        ...
        foreach ($files as $key => $file) {
            ...
            //保存文件 并记录保存成功的文件
            if ($this->save($file,$this->config['replace'])) {
    ...
    /**
     * 上传图片二进制格式
     * @param 文件信息二进制对象 $file
     */
    public function uploadImgBinary( $fileBinary ){
        if(empty($fileBinary)){
            $this->error = '没有上传的文件！';
            return false;
        }
        ...
        $imageName = $this->getSaveImgName();
        $fileResult = file_put_contents($this->config['rootPath'].$this->config['savePath'].$imageName, $fileBinary);
        if( empty( $fileResult ) ){
            return $fileResult;
        }
         
        return $this->config['savePath'].$imageName;
    }
     ...
     * @return string
     */
    private function getSaveImgName() {
        $imgName = md5(uniqid(mt_rand()));
        //文件保存后缀，支持强制更改文件后缀
        $ext = empty($this->saveExt) ? $this->config['saveExt'] : $this->saveExt;
        return $imgName.'.'.$ext;
    }
    ...
    private function getSaveName($file) {
        ...
        if (empty($rule)) { //保持文件名不变
            ...
            $savename = $file['sha1'];
        }else if('md5Time'==$rule){
            $savename = $file['md5'].time();
        }else if('hashTime'==$rule){
            $savename = $file['sha1'].time();
        } else{
